...
import com.github.shk0da.cloudconfig.web.rest.vm.EurekaVM;
...
public class EurekaResource {
    ...
    private Map<String, Object> getEurekaStatus() {
        ...
        String datacenter = ConfigurationManager.getDeploymentContext().getDeploymentDatacenter();
        try {
            datacenter += " :: " + InetAddress.getLocalHost().getHostName();
        } catch (Exception e) {
            log.warn("The host name could not be determined, skip this");
        }
        stats.put("datacenter", datacenter);
